...
export const SwooshOut = () => {
  ...
  let {
    ...
    error: requestError,
  } = useContractWrite(contract, "request");
  ...
  function handleMemberAdd(payAddress: string) {
    let address = payAddress.trim();
    if (
      address.length !== 42 ||
  ...
  async function submitRequest() {
    if (!Number.isNaN(parseFloat(amount))) {
      await requestMutateAsync({
        args: [members, ethers.utils.parseEther(amount), title, ""],
      })
        .then(() => {
          toast({
            title: "Swoosh Request Success!",
            status: "success",
            position: "bottom",
          });
        })
        .catch((error) => {
          ...
          toast({
            title: "Swoosh Request Unsuccessful",
            status: "error",
            position: "bottom",
          });
        });
  ...
  return (
    <div className=" p-8 rounded-lg bg-indigo-100 flex w-full flex-col">
      <div className="flex flex-col gap-2">
        ...
        <Card className="p-4  ">
          <InputGroup justifyContent="center" alignItems="center" gap={2}>
            ...
            <IconButton
              size="lg"
              onClick={() => handleMemberAdd(memberInput)}
              rounded={100}
        ...
        <div className="flex justify-between items-end gap-4 flex-wrap">
          ...
          <Button
            isDisabled={requestIsLoading}
            size="lg"
            colorScheme="sblue"
            className="flex-grow"
            onClick={submitRequest}
          >
            {requestIsLoading ? "Please Wait ..." : "SWOOSH"}
          </Button>
        </div>
      </div>
    </div>
